...
class FormulaAuditor
  ...
  def audit_line(line, lineno)
    ...
    if line =~ /depends_on :(.+) (if.+|unless.+)$/
      audit_conditional_dep(Regexp.last_match(1).to_sym, Regexp.last_match(2), $&)
    end

    if line =~ /depends_on ['"](.+)['"] (if.+|unless.+)$/
      audit_conditional_dep(Regexp.last_match(1), Regexp.last_match(2), $&)
    end
    ...
    if @strict
      ...
      if line =~ /(require ["']formula["'])/
        problem "`#{Regexp.last_match(1)}` is now unnecessary"
      end
    end
  end
